...
function refersion_admin_init() {
	...
	if ( empty( $refersion_status ) ) {
		add_action( 'admin_notices', 'refersion_warning' );
	}
...
function refersion_warning() {
	...
	$link = sprintf( __( '<a href="%1$s">Click here to configure the plugin</a>.', 'refersion-for-woocommerce' ), 'admin.php?page=refersion-config' );
...
function refersion_config() {
	...
	// Update options
	if ( ! empty($_POST) ) {
		update_option( 'refersion_status', sanitize_text_field ( $_POST['refersion_status'] ) );
		update_option( 'refersion_api_key', sanitize_text_field ( $_POST['refersion_api_key'] ) );
	}
...
?>
	...
	<div class="wrap">
		...
		<div style="width: 772px">
			...
			<div style="background: #eee">
				<div style="background: #ccc; padding: 10px; font-weight: bold"><?php _e( 'Configuration', 'refersion-for-woocommerce' ); ?></div>

				<form method="post" action="admin.php?page=refersion-config">

				<table class="form-table" style="margin: 10px">
					<tr>
						<th scope="row" style="white-space: nowrap"><label><?php _e( 'Tracking Status', 'refersion-for-woocommerce' ); ?>:</label></th>
						<td style="width: 100%">
							<select name="refersion_status">
								<option value="ENABLED" <?php if ( $refersion_status == "ENABLED" ) {?>selected="selected"<?php } ?>><?php _e ('Enabled', 'refersion-for-woocommerce' ); ?></option>
								<option value="DISABLED" <?php if ( $refersion_status == "DISABLED" ) {?>selected="selected"<?php } ?>><?php _e( 'Disabled', 'refersion-for-woocommerce' ); ?></option>
							</select>
						</td>
					</tr>
					<tr valign="top">
						<th scope="row" style="white-space: nowrap"><label><?php _e( 'Your Refersion Public API Key', 'refersion-for-woocommerce' ); ?>:</label></th>
						<td style="width: 100%">
							<input type="text" name="refersion_api_key" value="<?php echo $refersion_api_key; ?>" size="25" />
							<a href="https://refersion.uservoice.com/knowledgebase/articles/337317-where-can-i-find-my-refersion-public-api-key" target="_blank"><?php _e( 'Where do I find this?', 'refersion-for-woocommerce' ); ?></a>
						</td>
					</tr>
					<tr valign="top">
